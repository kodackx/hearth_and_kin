...
def test_delete_story_success(
    ...
    response = client.request('DELETE', 
                              f'/story/{story1.story_id}', 
                              json={'story_id': story1.story_id, 'character_id': characters[0].character_id}, 
                              headers=get_token[0])
...
def test_delete_story_not_found(client: TestClient, characters: list[Character], stories: list[Story], get_token: list[dict[str, str]]):
    story1 = stories[0]
    response = client.request('DELETE', 
                              f'/story/{story1.story_id + 1}', 
                              json={'story_id': story1.story_id + 1, 'character_id': characters[0].character_id}, 
                              headers=get_token[0])
...
def test_delete_story_not_creator(session: Session, client: TestClient, characters: list[Character], stories: list[Story], get_token: list[dict[str, str]]):
    ...
    _ = client.post(
        ...
        json={'story_id': story1.story_id, 'character_id': characters[1].character_id}, 
        headers=get_token[1]
    )
